...
const tableDataForty = utils.tableDataForty


const stdoutMessage = utils.stdoutMessage
const stdoutTable = utils.stdoutTable
...
const Weather = {
  ...
  fortyDataProxy: function () {
    ...
    return Promise.all([datas]).then((results) => {
        let originWeatherDayData = filterWeatherDataMonth(results[0].listFortyData)
        //组装日期 
        ...
        if(outFormatTable){
          ...
          console.log(chalk.cyan('**********40天天气预报**********'))
          console.log('\r')
